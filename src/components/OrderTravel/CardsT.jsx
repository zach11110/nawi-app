...
const CardsT = ({title}) => {
  ...
  return (
    <div className="min-h-screen text-right pt-12 bg-[#EBEAFA]">
      <main>
      <p className="p-6 mb-4 font-semibold text-right text-2xl mr-[160px]">{title}</p>
      
        <section className="flex flex-wrap justify-center max-w-7xl mx-auto py-3">

          { orders.length===0?  <div><Start/></div> :
            orders.map((order) => (
            <div key={order.id} className="w-full sm:w-1/2 lg:w-1/2 p-2 mt-[-20px]">
              <div className=" w-full rounded-xl shadow-md overflow-hidden mb-4 sm:mb-6 p-2 bg-white float-right">
                <div className="flex">
                  <img
                    className="w-[90px] h-[90px] object-cover mt-[25px]"
                    src={order.image}
                    alt="/"
                  />
                  <div className="p-4 flex flex-col justify-between float-right">
                    <div>
                      <div className="flex">
                        ...
                        <span className="text-yellow-500 mt-[-10px]">{order.rating}</span>
                      </div>
                      <div className="mt-[-15px] text-right">
                        <h3 className="text-[23px] font-semibold mr-[-21px]">
                          {order.name}
                        </h3>

                        <p className="text-gray-500 text-lg text-right p-2 ml-[300px]">
                          {order.location}
                        </p>
                        <svg
                          width="13"
                          ...
                          viewBox="0 0 30 36"
                          fill="none"
                          className="float-right mr-[-14px] mt-[-25px] mb-[-30px]"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                        ...
                        <p className="text-blue-700 text-md mr-[5px] text-[22px] font-semibold">
                          {order.price}
                        </p>
                      </div>
                    </div>
